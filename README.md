...
MPT_DATA_DIR=/data
MPT_MODEL_GATEWAY_BASE_URL=http://model-gateway.railway.internal:8080
```

`MPT_MODEL_GATEWAY_BASE_URL` 是模型网关管理/API 服务根地址，不要手动加 `/v1`。如果 Railway 里的模型网关服务名不是 `model-gateway`，把地址里的服务名替换成实际服务名。这个内网地址只由后端访问，不会展示给用户。
